...
class PosteApp(ft.Column):
    def init(self):
        # مقتبس من هندسة الكلاس المطور لضمان الاستقرار التام مع الأداة
        self.path_in = ft.TextField(label="مسار ملف الإكسيل المختار", hint_text="سيظهر المسار هنا بعد الاختيار 📁", width=400, read_only=True)
        ...
        # درس مستفاد من كود الشات: استخدام ListView مع التمرير التلقائي لمنع تجمد حركة الماوس للأسفل
        self.res_container = ft.ListView(expand=True, spacing=10, auto_scroll=True, height=400)
...
def main(page: ft.Page):
    ...
    # تأكيد تثبيت الوضع الفاتح لضرب سواد الويندوز والوضع الليلي تلقائياً
    ...
    app = PosteApp()
    page.overlay.append(app.file_picker) # ربط أداة الـ FilePicker بالصفحة الأساسية لفتح الملف بالماوس
    page.add(app)
    page.update()
