...
def main(page: ft.Page):
    ...
    path_in = ft.TextField(
        label="مسار ملف الإكسيل المختار",
        hint_text="سيظهر المسار هنا بعد الاختيار 📁",
        width=400,
        read_only=True,
    )

    search_in = ft.TextField(
    ...
    def start_process(e):
        ...
        try:
            ...
            res_df = df_clean.loc[
                df_clean.groupby("POSTE")["Total_I"].idxmax()
            ].copy()

            res_df["POSTE"] = res_df["POSTE"].replace(
                "853P", "P", regex=True
            )

            error_txt.value = (
                "✅ تم تحميل ومعالجة البيانات بنجاح! "
                "انتقل لتبويب البحث."
            )
            ...
            error_txt.value = (
                "❌ خطأ في مسار الملف أو صحة البيانات بداخله!"
            )
            error_txt.color = "red"
            print(f"ERROR: {ex}")
            page.update()
    ...
    def search_station(e):
        ...
        if res_df is None:
            res_container.controls.append(
                ft.Text(
                    "❌ يرجى تحميل ومعالجة البيانات من "
                    "التبويب الأول أولاً!",
                    color="red",
                )
            )
    ...
    def export_data(e):
        ...
        try:
            desktop_path = os.path.join(
                os.path.expanduser("~"),
                "Desktop",
                "Highest_Current_Stations_Report.xlsx",
            )
            ...
            error_txt.value = (
                "❌ فشل التصدير، تأكد من إغلاق ملف التقرير "
                "إذا كان مفتوحاً مسبقاً."
            )
            error_txt.color = "red"
    ...
    view_load = ft.Column(
        ...
        controls=[
            ...
            ft.Button(
                content=ft.Text("الدخول ونظام المعالجة"),
        ...
        controls=[
            ...
            ft.Row(
                [
                    search_in,
                    ft.Button(
                        content=ft.Text("ابحث"),
                        on_click=search_station,
                    ),
                ],
                alignment=ft.MainAxisAlignment.CENTER,
            ),
            ...
            ft.Button(
                content=ft.Text(
                    "تصدير التقرير المفلتر كاملاً إلى إكسيل 📄"
                ),
